...
def key(number1, number2, rules):
    rule1 = Rule(upper=number1, lower=number2)
    rule2 = Rule(upper=number2, lower=number1)

    return ((rule1 in rules) - (rule2 in rules))
...
for printing_order in data:
    if not is_valid(rules, printing_order):
        ordered_printing_order = sorted(printing_order, key=cmp_to_key(lambda a, b: key(a, b, rules)))

        total += ordered_printing_order[len(printing_order) // 2]
